    ...
    try {
      ...
      if (showToast) {
        toast.success('Productos actualizados correctamente', {
          description: `Se encontraron ${list.length} productos registrados.`
        })
      }
  ...
  const handleOpenModal = (product?: Product) => {
    ...
    if (product) {
      ...
      toast.info('Modo edición activado', {
        description: `Editando el producto "${product.name}".`
      })
    } else {
      setEditingId(null)
    ...
    if (confirm(`¿Estás seguro de que deseas eliminar "${name}"?`)) {
      ...
      try {
        ...
        toast.success('Producto eliminado exitosamente', {
          description: `"${name}" fue removido del inventario.`
        })
      } catch (error: any) {
        toast.dismiss(deletingToast)
    ...
    try {
      ...
      if (editingId) {
        ...
        toast.success('Producto registrado', {
          description: `"${formData.name}" fue añadido al inventario.`
        })
      }
      setIsModalOpen(false)
      await loadProducts()
