...
import { AlertCircle, FileText, Download, Loader2, Calendar as CalendarIcon } from 'lucide-react';
...
export function ExportPage() {
  ...
  const handleExportProduction = async () => {
    try {
      ...
      const dayData = await getDailyProduction(selectedDate, selectedDate);
      ...
      const [weekData, dayReject, weekReject] = await Promise.all([
      ...
      await generateProductionReport(products, measures, dataParaReporte);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8 animate-in fade-in duration-500 bg-background min-h-screen">
      <div>
        ...
        <p className="text-slate-500 font-medium">Genera documentos oficiales en formato Excel (.xlsx)</p>
      </div>
      ...
      <div className="bg-background border rounded-[2rem] p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <label className="text-[10px] uppercase font-black text-slate-400 tracking-widest">Fecha del Reporte</label>
            <input
              type="date"
              value={selectedDate}
        ...
        <div className="text-right hidden md:block">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-tighter">Estado del Servidor</p>
          <div className="flex items-center gap-2 text-green-500 justify-end">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
            </span>
            <span className="text-sm font-black uppercase">Conectado</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1  gap-6">
        <div className="group bg-background border-2 rounded-[2.5rem] p-8 transition-all duration-300 shadow-sm hover:shadow-xl">
          ...
          <h3 className="text-4xl font-extrabold mb-2">Producción Diaria</h3>
          <p className="text-slate-500 text-sm font-medium mb-8 leading-relaxed">
            Genera la hoja de balance detallada por categorías con acumulados semanales.
          </p>
