...
export function RejectProductsPage() {
    ...
    const loadRejects = async (showToast = false) => {

        try {

            setIsLoading(true)

            const response: any = await getRejectProducts(
            ...
            if (showToast) {
                ...
                toast.success('Registros actualizados', {
                    description: `Se encontraron ${dataList.length} rechazos registrados.`
                })
            }
            ...
            toast.error('Error al cargar registros', {
                description: error?.message || 'No fue posible consultar los rechazos.'
            })
    ...
    const handleOpenModal = (item?: RejectProduct) => {
        ...
        if (item) {
            ...
            toast.info('Modo edición activado', {
                description: `Editando el rechazo #${item.reject_productID}.`
            })
            ...
            toast.info('Nuevo rechazo', {
                description: 'Completa los datos para registrar una nueva merma.'
            })
        }
    ...
    const handleDelete = async (id: number) => {
        ...
        toast.warning('Confirmación requerida', {
            description: `Estás a punto de eliminar el registro #${id}.`
        })

        if (confirm(`¿Estás seguro de que deseas eliminar este registro de rechazo?`)) {

            const deletingToast = toast.loading('Eliminando registro...', {
                description: `Procesando eliminación del rechazo #${id}.`
            })
            ...
            try {
                ...
                toast.success('Registro eliminado', {
                    description: `El rechazo #${id} fue eliminado correctamente.`
                })
                ...
                toast.error('Error al eliminar registro', {
                    description: error?.message || 'No fue posible eliminar el rechazo.'
                })
            }
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const savingToast = toast.loading(
                editingId
                    ? 'Actualizando rechazo...'
                    : 'Registrando rechazo...',
                {
                    description: editingId
                        ? 'Guardando cambios en la base de datos.'
                        : 'Creando nuevo registro de merma.'
                }
            )

            if (editingId) {

                await updateRejectProduct(editingId, {
                ...
                toast.success('Registro actualizado', {
                    description: `El rechazo #${editingId} fue actualizado correctamente.`
                })

            } else {

                await createRejectProduct({
                    quantity: Number(formData.quantity)
                })
                ...
                toast.success('Rechazo registrado', {
                    description: `${formData.quantity} unidades fueron registradas como rechazo.`
                })
            }
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-muted-foreground text-sm font-medium uppercase tracking-tighter">
                                                        Unidades
                                                    </span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex justify-end gap-2">
                                                    <button
                                                        ...
                                                        className="p-2 border border-border hover:border-blue-500 hover:text-blue-500 rounded-lg transition-all active:scale-90"
                                                    >
                                                        <Edit2 size={18} />
                                                    </button>

                                                    <button
                                                        onClick={() => handleDelete(r.reject_productID)}
                                                        className="p-2 border border-border hover:border-red-500 hover:text-red-600 rounded-lg transition-all active:scale-90"
                                                    >
                                                        <Trash2 size={18} />
                                                    </button>
                                                </div>
                                    ...
                                    <tr>
                                        ...
                                        >
                                            ...
                                            <p className="text-lg font-medium">
                                                No hay registros de rechazo en este rango
                                            </p>
                                        </td>
                                    </tr>
                                )}
                ...
                <div className="p-2">
                    <div className="mb-6">
                        ...
                        <p className="text-muted-foreground text-sm">
                            Indica la cantidad de productos que no pasaron el control.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex gap-3 pt-4">
                            <button
                                type="submit"
                                disabled={isSaving}
                                className="flex-[2] btn-primary bg-destructive text-destructive-foreground h-12 uppercase font-black tracking-widest hover:opacity-90 shadow-lg disabled:opacity-70 disabled:cursor-not-allowed"
                            >
                                {isSaving ? (
